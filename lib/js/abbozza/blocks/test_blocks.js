...
Blockly.Blocks['params'] = { //declarement of global variables (used in test_main.compose/decompose)
  init: function() {
    this.appendDummyInput()
...
Blockly.Blocks['use_vars'] = { //use of variables of all kind in the programm
  ...
  init: function() {
    ...
    this.appendDummyInput()
        .appendField(new Blockly.FieldDropdown(function(){
            ...
            if(root==this){
                ...
                for(var i = 0; i < topBlocks.length; i++){
                    if(topBlocks[i].type=="global_vars_block"){
        ...
        },"VARS")); //hier einefnktion hin, die das dropdown macht
    ...
    this.appendValueInput("VAR")
     ...
     .appendField(new Blockly.FieldDropdown(function(){
            ...
            if(root==this){
                ...
                for(var i = 0; i < topBlocks.length; i++){
                    if(topBlocks[i].type="global_vars_block"){
                        this.symbols = topBlocks[i].symbols;
                        break;
                    }
                }
            ...
            var parent = this.symbols.getParent();
            while(parent){
                for(var i = 0; i < parent.getLength(); i++){
                   symbol = block.symbols.getSymbol2(i);
                   if(symbol.kind == Symbol.VARIABLE){
...
Blockly.Blocks['function'] = {
  ...
  compose: function(block){
    ...
    while(current){
       ...
       this.symbols.addSymbol(name,type,Symbol.PARAMETER);
       current = current.nextConnection.targetBlock();
  ...
  init: function() {
    ...
    this.appendDummyInput()
        ...
        .appendField(new Blockly.FieldDropdown(function(){
            ...
            for(var i = 0; i < block.symbols.getLength(); i++){
                   symbol = block.symbols.getSymbol2(i);
                   if(symbol.getType() == Symbol.OPERATION){
                        result = result.push([""+symbol.name,""+symbol.name]);
                   }
            }
