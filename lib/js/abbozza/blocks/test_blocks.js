...
Blockly.Blocks['test_main'] = {
  ...
  compose: function(block){
     ...
     this.symbols.reset();
      while(current){
        var name = current.getInputTargetValue("NAME");
        var type = current.getInputTargetValue("TYPE");
...
Blockly.Blocks['use_vars'] = { //use of variables of all kind in the programm
  ...
  init: function() {
    this.appendDummyInput()
        .appendField(new Blockly.FieldDropdown([["Variable", "Variable"]]), "VARIBALE"); //hier einefnktion hin, die das dropdown macht
