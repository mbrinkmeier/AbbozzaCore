  ...
  execute: function(interpreter,heapEntry){
   var context = interpreter.context_;
    ...
    if ( heapEntry.step_ == 0 ) {
        ...
        if(heapEntry.temp.matches(/yes/i)||heapEntry.temp.matches(/ja+/i)){
            block = this.getInputTargetBlock("yes");
            if(block){
                interpreter.enqueueBlock(block,heapEntry,heapEntry.symbols_);
            }
        }
        else if(heapEntry.temp.matches(/no+/i)||heapEntry.temp.matches(/nein/i)){
...
Blockly.Blocks['my_number'] = {
  ...
  execute: function(interpreter,heapEntry){
      numb = this.getInputTargetBlock("numb");
      heapEntry.finished_ = true;
      return numb;
  }
};
...
Blockly.Blocks['repeat'] = {
  ...
  execute :function(interpreter,heapEntry){
      if(heapEntry.step_ == 0){
          ...
          if(heapEntry.temp == heapEntry.result_){
              heapEntry.step_ = 2;
          }
      } else if (heapEntry.step ==  2){
          heapEntry.finished_ = true;
      }
      return null;
  }
